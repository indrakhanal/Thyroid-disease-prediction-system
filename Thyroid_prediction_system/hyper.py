...
class HyperThyroidism(Tk):
    def __init__(self):
        super(HyperThyroidism, self).__init__()
        self.title('Data Collection only for normal cases')
    ...
    def display_data(self):
        ...
        con.cursor()
        all_data = con.execute("select * from Patient where result='Hyperthyroidism'")
        return all_data

    def show_normal(self):
        label_frame2 = LabelFrame(self, text=f"Result of patient With HyperThyroidism Result", font=("monaco", 16, 'bold'))
        label_frame2.grid(column=0, row=0)

        trees = ttk.Treeview(label_frame2, column=("Name", "Age ", "Result", "tsh", "t3", "tt4", "t4u", "fti"),
                            show='headings')

...
class Normal(Tk):
    ...
    def display_data(self):
        ...
        con.cursor()
        all_data = con.execute("select * from Patient where result='Normal'")
        return all_data

    def show_normal(self):
        label_frame = LabelFrame(self, text=f"Result of patient With Normal Result", font=("monaco", 16, 'bold'))
        label_frame.grid(column=0, row=0)

        trees = ttk.Treeview(label_frame, column=("Name", "Age ", "Result", "tsh", "t3", "tt4", "t4u", "fti"),
                            show='headings')
